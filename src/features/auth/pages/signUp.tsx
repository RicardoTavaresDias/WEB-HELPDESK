...
export function Signup(){
  ...
  return (
    <>
      ...
      <Alert severity="success" open={!!errors.root?.success}>{errors.root?.success as string}</Alert>
          ...
          <div id="animeAuth" className="w-110 px-4 2xl:px-0" >
            ...
            <form onSubmit={handleSubmit(onSubmit)} >
              <Form textTitle="Crie sua conta" subtitle="Informe seu nome, e-mail e senha" textButton="Cadastrar" isLoading={isSubmitting}>
                <Input {...register("name")} type="text" label="nome" placeholder="Digite o nome completo" error={errors.name && errors.name.message} autoComplete="current-nome" />
                <Input {...register("email")} type="text" label="e-mail" placeholder="exemplo@mail.com" error={errors.email && errors.email.message} autoComplete="current-e-mail" />
                <Input {...register("password")} type="password" label="senha" placeholder="Digite sua senha" textLabel="Mínimo de 6 dígitos" error={errors.password && errors.password.message} autoComplete="current-password" />
              </Form>
            </form>
